...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    .AddCookie(options =>
    {
        // Esta es la ruta a tu página de login. El sistema redirigir aquí.
...
{
    // Pol�tica para "Admin": Requiere el claim ("permiso", "admin")
    options.AddPolicy("AdminPolicy", policy =>
        policy.RequireClaim("permiso", "admin"));

    // Pol�tica para "Sprfm": Requiere el claim ("permiso", "sprfm")
    options.AddPolicy("SprfmPolicy", policy =>
        policy.RequireClaim("permiso", "sprfm"));

    // Pol�tica para "Siisu": Requiere el claim ("permiso", "siisu")
    options.AddPolicy("SiisuPolicy", policy =>
        policy.RequireClaim("permiso", "siisu"));

    // Pol�tica "O": Requiere "sprfm" O "siisu"
